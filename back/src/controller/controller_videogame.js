...
// Définir la fonction qui utilisera la méthode GET pour récupérer les éléments du tableau videogame
exports.getDataVideoGame = (request, response) => {
    ...
    if (err) {
      ...
      // Sinon renvoi status 200, et les données en format json
        ...
        response.status(200).json(JSON.parse(data).videogame);
    }
    });
};
...
exports.getDataById = (request, response) => {
    ...
    // Condition si erreur 500
        if (err) {
            ...
            const dataId = manipData.videogame.find(
            ...
            // sinon
                ...
                response.status(404).json({
                    message:"Vous devez faire erreur, aucun jeu vidéo ne correspond à cet id !",
                    error: err,
                });
            }
        }
...
exports.createData = (request, response) => {
    ...
    fs.readFile("./src/model/tableau.json", (err, data) => {
        ...
        if (err) {
            ...
            let arrayLength = existingData.videogame.length;
            // Si le tableau n'est pas vide
            if (arrayLength > 1) {
                // Récupère le dernier objet du tableau boissons
                let lastObject = existingData.videogame[arrayLength - 1];
                // Créer et insérer la new data avec l'id = à celle du dernier objet +1
                existingData.videogame.push({
                    id: lastObject.id + 1,
            ...
            fs.writeFile("./src/model/tableau.json", JSON.stringify(existingData),(writeErr) => {
                ...
                response.status(500).json({
                    message: "Impossible de créer une entrée pour ce jeu vidéo !",
                    error: err,
                });
                // sinon
                } else {
                    ...
                    response.status(200).json({
                        message: "Ce jeu vidéo a bien été rajouté.",
                    });
                }
            });
        }
...
exports.updateData = (request, response) => {
    ...
    fs.readFile("./src/model/tableau.json", (err, data) => {
        ...
        if (err) {
            ...
            const dataId = existingData.videogame.find(
                (obj) => obj.id === parseInt(request.params.id)
            );
            ...
            // 'Sinon' on trouve l'objet donc ->
                ...
                fs.writeFile("./src/model/tableau.json", JSON.stringify(existingData), (writeErr) => {
                    ...
                    // sinon status 200 succes message
                        ...
                        response.status(200).json({
                            message: "Ce jeu vidéo a bien été modifié !",
                        });
                    }
                });
            }
    ...
    fs.readFile("./src/model/tableau.json", (err, data) => {
        ...
        // Sinon
            ...
            // Sinon
                ...
                existingData.videogame = existingData.videogame.filter(
                    (obj) => obj.id != parseInt(request.params.id)
                );
                ...
                fs.writeFile("./src/model/tableau.json", JSON.stringify(existingData), (writeErr) => {
                    if (writeErr) {
                        ...
                        response.status(200).json({
                            message: "Ce jeu vidéo a bien été supprimé. !",
                        });
                    }
                });
            }
